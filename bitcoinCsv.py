...
# df['ds'] = df.loc[100:]['Date']
# df['y'] = df.loc[100:]['Close']
# data = df[['ds','y']]
# data.drop(data.index[:100],inplace=True) #0~100까지 날림/ inplace로 바로 적용
...
data = df[['ds','y']]
data.drop(data.index[300:],inplace=True) #0~100까지 날림/ inplace로 바로 적용 #위아래 순서중요*
